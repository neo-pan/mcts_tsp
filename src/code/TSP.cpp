...
void Solve_One_Instance(int Inst_Index)
{
    ...
    else if (Current_Solution_Double_Distance / Magnify_Rate - Stored_Solution_Double_Distance / Magnify_Rate >
             0.000001)
    ...
    // printf("\nInst_Index:%d Concorde Distance:%f, MCTS Distance:%f
    // Improve:%f Time:%.2f Seconds\n", Inst_Index+1,
    // Stored_Solution_Double_Distance/Magnify_Rate,
    // 		Current_Solution_Double_Distance/Magnify_Rate,
    // Stored_Solution_Double_Distance/Magnify_Rate-Current_Solution_Double_Distance/Magnify_Rate,
    // ((double)clock()-Current_Instance_Begin_Time)/CLOCKS_PER_SEC);

    FILE *fp;
    fp = fopen(Statistics_File_Name, "a+");
    fprintf(fp,
            "\nInst_Index:%d \t City_Num:%d \t Concorde:%f \t MCTS:%f "
            "Improve:%f \t Time:%.2f Seconds\n",
...
bool Solve_Instances_In_Batch()
{
    ...
    if (FIC.fail())
    {
        ...
        ; // cout << "\n\nBegin to read instances information from
    ...
    for (int i = 0; i < Total_Instance_Num; i++)
    {
        for (int j = 0; j < Temp_City_Num; j++)
        {
            ...
            {
                FIC >> Temp_Distance;
                Stored_Distances[i][j][k] = Temp_Distance;
            }
        }
    ...
    for (int i = Index_In_Batch * Inst_Num_Per_Batch;
         i < (Index_In_Batch + 1) * Inst_Num_Per_Batch && i < Total_Instance_Num; i++)
...
int main(int argc, char **argv)
{
    ...
    Heatmap_File_Name = argv[6];

    Alpha = atof(argv[7]);
    Beta = atof(argv[8]);
    Param_H = atof(argv[9]);
    Param_T = atof(argv[10]);
    Max_Candidate_Num = atoi(argv[11]);
    Candidate_Use_Heatmap = atoi(argv[12]);
    Max_Depth = atoi(argv[13]);
    ...
    fp = fopen(Statistics_File_Name, "a+");
    fprintf(fp,
            "\n\nAvg_Concorde_Distance: %f Avg_MCTS_Distance: %f Avg_Gap: %f "
            "Total_Time: %.2f Seconds \n Beat_Best_Known_Times: %d "
            "Match_Best_Known_Times: %d Miss_Best_Known_Times: %d \n",
            Sum_Opt_Distance / Test_Inst_Num, Sum_My_Distance / Test_Inst_Num, Sum_Gap / Test_Inst_Num,
            ((double)clock() - Overall_Begin_Time) / CLOCKS_PER_SEC, Beat_Best_Known_Times, Match_Best_Known_Times,
            Miss_Best_Known_Times);
    fclose(fp);
    ...
    // Total_Time: %.2f Seconds \n Beat_Best_Known_Times: %d
    // Match_Best_Known_Times: %d Miss_Best_Known_Times: %d \n",
    // 		Sum_Opt_Distance/Test_Inst_Num,Sum_My_Distance/Test_Inst_Num,
    // Sum_Gap/Test_Inst_Num,
    // ((double)clock()-Overall_Begin_Time)/CLOCKS_PER_SEC,
    // Beat_Best_Known_Times, Match_Best_Known_Times, Miss_Best_Known_Times);
    // getchar();
